...
def handle_client(sock, addr):
    ...
    try:
        ...
        broadcast(room, {"type": "system", "msg": f"{username} joined the room", "time": timestamp()})
        # Main message loop
        while True:
            ...
            for line in raw.split("\n"):
                ...
                if msg["type"] == "msg":
                    ...
                    if parts[0] == "/users":
                        ...
                        names = [info["username"] for client, info in clients.items() if info["room"] == room]
                        sock.sendall((json.dumps({"type": "system", "msg": f"Users in {room}: {', '.join(names)}", "time": timestamp()}) + "\n").encode())
                    elif parts[0] == "/allrooms":
                        # List all active rooms
                        rooms_list = list(set([info["room"] for info in clients.values()]))
                        sock.sendall((json.dumps({"type": "system", "msg": f"Active rooms: {', '.join(rooms_list)}", "time": timestamp()}) + "\n").encode())
                    elif parts[0] == "/pm" and len(parts) >= 3:
                        target = parts[1]
                        text = " ".join(parts[2:])
                        if send_private(sock, target, text):
                            sock.sendall((json.dumps({"type": "system", "msg": f"PM sent to {target}", "time": timestamp()}) + "\n").encode())
                        else:
                            sock.sendall((json.dumps({"type": "system", "msg": f"User {target} not found", "time": timestamp()}) + "\n").encode())
                    elif parts[0] == "/join" and len(parts) >= 2:
                        old_room = room
                        new_room = parts[1]
                        # Notify old room
                        broadcast(old_room, {"type": "system", "msg": f"{username} left the room", "time": timestamp()})
                        # Update room
                        clients[sock]["room"] = new_room
                        room = new_room
                        # Notify new room
                        broadcast(new_room, {"type": "system", "msg": f"{username} joined the room", "time": timestamp()})
                        sock.sendall((json.dumps({"type": "system", "msg": f"Joined room: {new_room}", "time": timestamp()}) + "\n").encode())
                        log(old_room, f"[{timestamp()}] {username} left for {new_room}")
                        log(new_room, f"[{timestamp()}] {username} joined from {old_room}")
                    elif parts[0] == "/help":
                        help_text = "Commands: /users, /allrooms, /pm <user> <msg>, /join <room>, /help"
                        sock.sendall((json.dumps({"type": "system", "msg": help_text, "time": timestamp()}) + "\n").encode())
    except Exception as e:
        ...
        if sock in clients:
            ...
            del clients[sock]
            broadcast(room, {"type": "system", "msg": f"{username} left the room", "time": timestamp()})
...
def start_server():
    ...
    print(f"Clients should connect to: {socket.gethostbyname(socket.gethostname())}:{PORT}")
    try:
        while True:
            ...
            if addr[0] in banned_ips:
                try:
                    client_sock.sendall((json.dumps({"type": "system", "msg": "Your IP is banned.", "time": timestamp()}) + "\n").encode())
                    client_sock.close()
                except:
                    pass
                continue
