...
class ChatClient:
    ...
    def create_widgets(self):
        ...
        sidebar_frame = tk.Frame(main_frame, bg="white", relief=tk.RAISED, bd=1, width=150)
        ...
        self.text_area.tag_config("system", foreground="#3498db", font=("Courier", 9, "bold"))
        self.text_area.tag_config("private", foreground="#e74c3c", font=("Courier", 9, "bold"))
        self.text_area.tag_config("timestamp", foreground="#95a5a6", font=("Courier", 8))
        self.text_area.tag_config("sender", foreground="#27ae60", font=("Courier", 9, "bold"))
    ...
    def connect_to_server(self):
        try:
            ...
            server_ip = simpledialog.askstring("Server Connection", "Enter server IP address (e.g., 192.168.1.100):")
            if not server_ip:
                self.root.destroy()
                return
            ...
            self.display_message("✓ Connected to server! Type /help for available commands.", "system")
            self.display_message("─" * 60, "system")
    ...
    def handle_message(self, msg):
        ...
        if msg["type"] == "msg":
            ...
            sender = msg.get("from", "Unknown")
            text = msg.get("msg", "")
            self.display_message(f"[{timestamp}] 🔒 [PM from {sender}] {text}", "private")
